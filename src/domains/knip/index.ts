...
/** Marker written into every generated config; its absence means hand-written. */
const GENERATED_MARKER = "Generated by @adddog/monorepo-consistency";
...
const generate = async (options: KnipGenerateOptions): Promise<KnipGenerateResult> => {
    ...
    try {
        ...
        for (const pkg of workspace.packages) {
            ...
            // Never overwrite hand-written configs (those lacking the generated
            // marker), even with --force. Protects manually-maintained configs
            // (custom root workspaces, bespoke entry/ignore) from being clobbered.
            if (options.force && existsSync(knipConfigPath)) {
                const existing = await readFile(knipConfigPath, "utf8");
                if (!existing.includes(GENERATED_MARKER)) {
                    logger.debug(`  ${pkg.name}: preserved (hand-written, no generated marker)`);
                    skipped++;
                    continue;
                }
            }
